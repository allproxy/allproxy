...
export class FreshFirefox implements Interceptor {
    ...
    async startFirefox(
        ...
        browser.stop = async function () {
            if (process.platform === "win32") {
                ...
                await windowsKill(`CommandLine Like '%\\\\firefox.exe%${initialUrl}'`).catch(console.error);
            } else {
                normalStop();
            }
        };
    ...
    async setupFirefoxProfile() {
        ...
        let messageShown: Promise<void> | true = messageServer.waitForSuccess().catch(console.error);
    ...
    async activate(proxyPort: number) {
        ...
        existingPrefs = (prefContents)
            .split('\n')
            .reduce((prefs: _.Dictionary<any>, line: string) => {
        ...
        await certCheckServer.start("https://google.com");
        ...
        browser.process.once('close', async (exitCode: any) => {
